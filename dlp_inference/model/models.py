...
from .modules import DLPEncoder, DLPDecoder, DLPContext
from .modules import DLPDynamics
# util functions
...
class DLP(nn.Module):
    def __init__(self,
                 ...
                 share_bg_across_time=False,  # Share one z_bg per temporal window (avg posterior over T)
        ...
        self.share_bg_across_time = share_bg_across_time  # one z_bg per temporal window
        assert not (share_bg_across_time and features_dist == 'categorical'), \
            "share_bg_across_time pools gaussian posterior params (mu/logvar); " \
            "averaging categorical bg logits is not supported"
    ...
    def forward(self, x, deterministic=False, warmup=False, with_loss=False, beta_kl=0.1, beta_dyn=0.1,
        ...
        z_bg_features = enc_dict['z_bg_features']
        if self.share_bg_across_time and timestep_horizon > 1 and mu_bg_features is not None:
            # One background latent per temporal window: average the per-frame
            # posterior parameters over T, draw a single sample, and broadcast
            # it back so every frame is decoded with the same z_bg. The
            # broadcasted mu/logvar also make the bg KL count once per window.
            same_params = logvar_bg_features is mu_bg_features
            mu_bg_features = mu_bg_features.mean(dim=1, keepdim=True)
            if same_params:
                logvar_bg_features = mu_bg_features
            elif logvar_bg_features is not None:
                logvar_bg_features = logvar_bg_features.mean(dim=1, keepdim=True)
            if deterministic or logvar_bg_features is None or same_params:
                z_bg_shared = mu_bg_features
            else:
                z_bg_shared = reparameterize(mu_bg_features, logvar_bg_features)
            expand_t = (-1, timestep_horizon) + (-1,) * (mu_bg_features.dim() - 2)
            mu_bg_features = mu_bg_features.expand(*expand_t).contiguous()
            if logvar_bg_features is not None:
                logvar_bg_features = logvar_bg_features.expand(*expand_t).contiguous()
            z_bg_features = z_bg_shared.expand(*expand_t).contiguous()
    ...
    def calc_static_elbo(self, x, model_output, warmup=False, beta_kl=0.05, beta_dyn=1.0, beta_rec=1.0,
        ...
        batch_size = x.shape[0]
        # actual number of frames in the input (can exceed self.timestep_horizon,
        # e.g. temporal windows fed to a static model for bg sharing); every
        # frame is treated as an independent sample for recon/particle terms
        ...
        if use_kl_mask:
            kl_mask = obj_on.reshape(-1, obj_on.shape[2])  # [bs * T, n_kp]
        ...
        # [bs, T, n_kp, 2] -> [bs * T, n_kp, 2], matching kp_p's flat layout
        mu_post = mu_anchor.reshape(-1, *mu_anchor.shape[-2:])
        logvar_post = torch.zeros_like(mu_post)
        loss_kl_kp_base = kl_loss_func(mu_preds=mu_post, logvar_preds=logvar_post, mu_gts=mu_prior,
                                       logvar_gts=logvar_prior)  # [batch_size, ]
